...
export default function Navbar() {
  ...
  // On mount, read isAuthenticated from localStorage
  useEffect(() => {
  ...
  const navLinks = [
    isAuthenticated ? { label: 'User Dashboard', href: '/user/dashboard' } : { label: 'Home', href: '/' },
          ...
          <>
            ...
            {dropdownOpen && (
              <div className="absolute right-0 top-12 mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-100 z-50 py-2">
                ...
                <button className="block w-full text-left px-5 py-2 text-red-600 hover:bg-red-50 transition-all" onClick={handleSignOut}>Sign Out</button>
              </div>
            )}
          </>
        ) : (
